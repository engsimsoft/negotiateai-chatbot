...
- **Этап:** Этап 1 / 4
- **Прогресс:** 12/78 задач (15%)
- **Следующее:** Фаза 1.4 — Упрощение text артефакта
...
- [ ] Обновить `textPrompt` в `lib/ai/prompts.ts`:
  ```
  Новые правила:
  - Генерировать plain text БЕЗ Markdown разметки
  - Использовать emoji вместо буллетов (✅ 📌 🔹 вместо - * 1.)
  - Использовать пустые строки для разделения вместо ## заголовков
  - Текст должен идеально копироваться в VK/Telegram/Instagram
  ```
- [ ] Упростить UI в `artifacts/text/client.tsx`:
  - Убрать Markdown рендеринг (показывать как plain text)
  - Оставить кнопку "Copy"
- [ ] Добавить кнопку "Download" (.txt файл)
- [ ] Решить: убрать AI suggestions или оставить?
- [ ] Тестирование: создать text артефакт, скопировать в VK

---

#### Фаза 1.5: Очистка и проверка (1-2 часа)

- [ ] Удалить неиспользуемые imports во всех изменённых файлах
- [ ] Проверить TypeScript: `npm run build` без ошибок
- [ ] Тест: создание text артефакта работает
- [ ] Тест: старые text артефакты открываются
- [ ] Commit: `refactor: remove code/sheet artifacts, simplify text (v2.2.0)`
...
- [X] Build успешен
- ⏸️ Следующее: Фаза 1.4 — Упрощение text артефакта
