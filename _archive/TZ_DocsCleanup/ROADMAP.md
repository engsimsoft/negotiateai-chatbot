...
|---|---|
| Этапов | 5 |
| Текущий этап | **5/5 ✅ закрыты по коду/docs** (1 ✅ commit, 2+3+4+5 🟡 ждут final manual test + commit + архивация) |
| Сессий (факт) | 2 (Сессия 1 = Этап 1 + WORKFLOW mini-ext; Сессия 2 = Этапы 2+3+4+5 + RAG-split фикс + SSOT-фикс). Планировали 3, уложились в 2. |
...
## Этап 2: `docs/architecture.md` trim + пофайловая карта + UI SSOT

**Статус:** 🟡 Расширенный scope выполнен, ждём мануальный тест Владимира

**Цель (финальная, с учётом пост-ревью Владимира):** Архитектурный документ = **пофайловая карта для поиска**, куда можно заходить из CLAUDE.md и быстро находить ответ на «где лежит код для X» + «какие компоненты уже есть до того как я начну делать UI с нуля».

**Задачи (базовый trim):**

- [x] Прочитать `docs/architecture.md` целиком, карта секций.
- [x] Убрать 21 версионный тег (v3.83, v3.58, v3.69, …) из ASCII-диаграммы слоёв и описаний модулей.
- [x] Убрать 5 ТЗ-пометок.
- [x] Описание слоёв сверено с `ls app/`, `ls lib/` — добавлены недостающие route groups и папки.
- [x] External Services обновлён: xAI primary + Anthropic + MiniMax + Gemini reserved.
- [x] Удалена фантазийная секция «Smart Routing (план)».
- [x] «Anthropic Claude (основной)» → «Мультипровайдерная маршрутизация» со ссылками.

**Задачи (расширение пост-ревью — ключевое):**

- [x] Добавлено **оглавление** + быстрый список SSOT-файлов.
- [x] **⛔ UI-секция как H2 сразу после TOC** — 25 shadcn/ui примитивов + 17 AI-chat элементов + фичевые папки + 4-шаговое правило при новом UI. Защита от «изобретаю компонент с нуля».
- [x] `#### lib/ai/tools/` переписан: 6 → **17 файлов**, сгруппированы по 5 категориям (reg, web/research, artifacts, context/files, utility).
- [x] **Data Layer полностью переработан:** 19 → **29 таблиц**, 7 категорий с H4. **Добавлены все MIND-таблицы** (пропускать их было критической ошибкой).
- [x] **Новая H2 «Карта фич (пофайлово)»** — для 13 фич route + pipeline + tools + UI + БД + модели + ADR.
- [x] Сломанная иерархия H3 «### 5. Streaming Pipeline» под H2 «Система промптов» исправлена — промотано до отдельной H2.
- [x] Дубликат Prompt System убран — раздел существовал дважды.
- [x] ASCII route groups уточнён.
- [x] Связанные документы: 6 → 11 ссылок (добавлены design-system, ai-chats-map, model-catalog-ops, troubleshooting, mcp-tools).

**Файлы:**
- `docs/architecture.md` — редактирование (**366 → 542 строк, +176 строк чистой навигации**)

**Валидация этапа:**

- [x] `grep -c 'v3\.' docs/architecture.md` → **0** ✅
- [x] `grep -c 'ТЗ-' docs/architecture.md` → **0** ✅
- [x] БД таблицы: 29/29 = `grep pgTable lib/db/schema.ts` ✅
- [x] tools/ файлы: 17/17 = `ls lib/ai/tools/` ✅
- [x] components/ui + components/elements + components/shared перечислены полностью ✅
- [x] Иерархия заголовков: нет orphan H3 ✅
- [ ] 🧪 **Мануальная проверка Владимиром** (ждём).
...
**Статус:** 🟡 Полностью переписан (2323 → 165 строк), ждём мануальный тест Владимира

**Выполнено в Сессии 2:** файл полностью переписан, все задачи ниже закрыты. Валидация:
- `wc -l` → **165** ✅ (было 2323)
- `grep 'v3\.'` → **0** ✅
- `grep 'ТЗ-'` (русское) → **0** ✅
- Версия = package.json 3.91.0 ✅
- SSOT compliance audit пройден (6 разделов)

Полный лог правок: [CHANGELOG.md](CHANGELOG.md) → «Этап 3 выполнен». Контекст передачи: [HANDOFF.md](HANDOFF.md) → верхняя запись.
...
**Статус:** 🟡 Выполнен полностью (Сессия 2, 2026-04-15). Mini-extension закрыт в Сессии 1, полный этап — в Сессии 2. Ждёт financial manual test + commit.

**Результат:** Правило 6 таблица расширена с 3 до **13 строк** (все docs/ файлы с file-triggers). Финализация чек-лист расширен с 2 до 13 docs с explicit triggers. WORKFLOW.md 746 → 791 строк. Валидация: `for f in 13-docs; do grep $f WORKFLOW; done` → **13/13 ✅**. Полный лог — [CHANGELOG.md](CHANGELOG.md) → «Этап 4».
...
**Статус:** 🟡 Выполнен полностью (Сессия 2, 2026-04-15). Ждёт financial manual test + commit + архивация.

**Результат:**
- `docs/ai-minimax.md` — banner добавлен (256 → 271 строк). Отличие от образца в этом ROADMAP: **не полный archival**, а частичный — MiniMax всё ещё активен для `create` chatMode и briefing pipeline (filter/author/section long-timeout + podcast-script).
- Главный `CHANGELOG.md` — запись «TZ_DocsCleanup (5 этапов)» добавлена в `[Unreleased]` после CLAUDE.md cleanup блока.
- Финальная валидация — все 7 правленных файлов проверены, CLAUDE.md = 211 (инвариант ≤ 220 ✅), 0 версионных тегов в теле ключевых docs, 13/13 docs упомянуты в WORKFLOW.

Полный лог — [CHANGELOG.md](CHANGELOG.md) → «Этапы 4 и 5 выполнены».
