...
**Задачи:**
- [x] Создать компонент `components/meeting/meeting-result.tsx`:
...
- [x] Создать компонент `components/meeting/meeting-list.tsx`:
...
- [x] Добавить API route `app/(chat)/api/meeting/records/route.ts` (GET: список записей пользователя)
- [x] Добавить API route `app/(chat)/api/meeting/records/[id]/route.ts` (GET: конкретная запись, DELETE: удаление)
- [x] Интегрировать list + result в meeting-page.tsx (state machine: empty → recording/uploading → processing → result, + навигация к старым записям)
- [x] Обновить meeting-card.tsx: показывать количество записей / последнюю запись
...
- [x] `npx tsc --noEmit` — 0 ошибок
- [x] `npm run build` — успешен
- [x] Браузер: после обработки — виден markdown документ с метаданными
- [x] Браузер: тайм-коды в детальном режиме кликабельны → аудио перематывается
- [x] Браузер: список записей показывает предыдущие записи
- [x] Браузер: тап на запись → загружается результат из БД
- [x] Браузер: удаление записи работает
- [x] Браузер: карточка на дашборде отражает наличие записей
- [x] 🧪 Мануальный тест пользователем (полный E2E flow)
