...
#### Header:
- [ ] **4.1** Убрать dropdown выбора агента
- [ ] **4.2** Убрать badge модели
- [ ] **4.3** Убрать Private/Public toggle
- [ ] **4.4** Добавить постоянные кнопки [📝] и [❓]
- [ ] **4.5** Новая структура: Simply logo | + Новый чат | ... | [📝] [❓] [👤]

#### Chat:
- [ ] **4.6** Убрать @-mentions из `multimodal-input.tsx` (отключить MentionAutocomplete)
- [ ] **4.7** Убрать guest styling из `message.tsx` (отступ, фон, метка "↩️ гость")

#### Sidebar:
- [ ] **4.8** Убрать `SidebarAgents` из `app-sidebar.tsx`

#### Роутинг:
- [ ] **4.9** Удалить `app/(chat)/agents/` — весь каталог
- [ ] **4.10** Настроить редирект /agents → /chat (или 404)
...
- [ ] **6.1** Создать SQL миграцию
- [ ] **6.2** Выполнить миграцию на dev БД
- [ ] **6.3** Проверить что таблицы удалены
- [ ] **6.4** Проверить что поле `hasSeenBenIntro` добавлено
...
#### Автоматическое:
- [ ] **8.1** `npm run build` — без ошибок
- [ ] **8.2** `npm run lint` — без ошибок

#### Мануальное:
- [ ] **8.3** Сценарий: Prompt-агент
  ```
  1. Нажать [📝] в header
...
### Must have:
- [ ] Таблицы `agent` и `userAgent` удалены из БД
- [ ] Поля `agentId` удалены из `chat` и `message`
- [ ] Поле `hasSeenBenIntro` добавлено в `user`
