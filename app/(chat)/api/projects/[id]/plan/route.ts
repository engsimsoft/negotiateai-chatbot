...
export async function POST(
  ...
  try {
    ...
    // streamText обязателен — cap SSOT для professor:planning = 32000 > Anthropic
    // non-streaming threshold 21333 (ADR «AI SDK invocation contract»).
    // .text автоматически consumes stream и резолвится полным текстом;
    // .usage к этому моменту уже готов.
    // Anthropic SDK (@ai-sdk/anthropic) не принимает temperature совместно с
     // thinking — выдаёт warning и отключает thinking. Поэтому temperature
     // передаётся ТОЛЬКО когда модель не поддерживает thinking (например Haiku).
     // budgetTokens = 16000 (половина maxOutputTokens 32000) даёт Opus
     // детерминированный reasoning-бюджет для сложных 10+ задач.
    const result = streamText({
      model: getModel("professor:planning"),
      maxOutputTokens: getMaxOutputTokensForTask("professor:planning"),
      ...
      ...(supportsThinking
        ? {
            providerOptions: {
              anthropic: {
                thinking: { type: "enabled" as const, budgetTokens: 16000 },
              },
            },
          }
        : {
    ...
    const planReport = extractTag(text, "plan_report");
    const planJsonRaw = extractTag(text, "plan_json");
    ...
    if (!planReport || !planJsonRaw) {
      ...
      console.error("[ProfessorPlan] Raw response:", text.slice(0, 500));
      return NextResponse.json(
