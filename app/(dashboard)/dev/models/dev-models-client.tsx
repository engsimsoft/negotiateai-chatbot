...
// ---------------------------------------------------------------------------
// Compatibility heuristic — vision tasks need vision-capable models, etc.
// ---------------------------------------------------------------------------
...
function taskRequiresCapability(
  ...
  // Vision routing tasks (simply-chat-vision, vision:ocr, artifact that takes images)
  if (taskId.includes("vision") && !caps.vision) {
    return { ok: false, reason: "task handles images — model lacks vision" };
  }
  // Embedding tasks — e.g. memory:extract is an LLM task NOT an embedding task;
  // only `embed` prefix tasks need embedding models. None exist right now, so
...
// ---------------------------------------------------------------------------
// Model <select> — all catalog entries, grouped by provider, with warning
// ---------------------------------------------------------------------------
...
function ModelSelect({
  ...
  return (
    <Select value={value} onValueChange={onChange} disabled={disabled}>
      <SelectTrigger className="h-8 w-full max-w-[300px] font-mono text-xs">
        <SelectValue />
      </SelectTrigger>
