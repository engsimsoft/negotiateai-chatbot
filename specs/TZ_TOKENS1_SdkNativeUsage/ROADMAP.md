...
**Статус:** ✅ Все этапы завершены (v3.67.0)
...
### Этап 8: Валидация (3 основных типа чатов)

**Статус:** ✅ Завершён
**Цель:** Прогнать аудит по chat/expertise/create, сверить с Anthropic Console, допуск <1%.
**Scope:** Только основные чаты (chat, expertise, create). Pipelines (briefing, meeting, project) и service chats (ben, manager) — отдельное ТЗ.
...
| # | Тип | chatMode | Модель | Вызовов | DB costUsd | Расчёт Anthropic | Δ% | Статус |
|---|-----|----------|--------|---------|------------|------------------|-----|--------|
| 1 | Обычный чат | chat | Haiku 4.5 | 7 | $0.0453 | $0.0454 | 0.16% | ✅ |
| 2 | Экспертиза | expertise | Sonnet 4.6 | 3 | $0.0521 | $0.0521 | 0.06% | ✅ |
| 3 | Создание | create | Sonnet 4.6 | 2 | $0.0418 | $0.0416 | 0.40% | ✅ |

**Типы 4-7 (service chats, briefing, meeting, project) — вынесены в отдельное ТЗ.**

**8.3 — Прогон по типам:**
- [x] Тип 1: 7 вызовов в chat (Haiku), мануальный + SQL тест
- [x] Тип 2: 3 вызова в expertise (Sonnet), мануальный + SQL тест
- [x] Тип 3: 2 вызова в create (Sonnet), мануальный + SQL тест

**8.4 — Анализ:**
- [x] Все 3 строки таблицы заполнены
- [x] Δ% < 1% во всех случаях (max 0.40%)
- [x] NULL costUsd = 0 для всех 3 типов (100% coverage)

**Валидация этапа:**
- [x] Таблица расхождений заполнена
- [x] Все Δ% < 1%
- [x] `npm run build` → успешен (Этап 7 build)

**Критерий готовности:** 3/3 основных типа прошли валидацию, расхождений нет.
