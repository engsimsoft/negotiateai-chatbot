...
|---|---|---|---|
| [TZ_BriefingAuthorUrlHallucination](TZ_BriefingAuthorUrlHallucination.md) | **Briefing author выдумывает 82-91% URL — все модели одинаково (Sonnet, Gemini, MiniMax, Grok 4.20 non-reasoning).** Empirical test 2026-04-16: MiniMax 10/11 vs Grok 4.20 9/11 — marginal difference, смена модели не решает. Исторический контекст: метрика `fabricated` добавлена в проект специально потому что Sonnet и Gemini в этой роли тоже галлюцинировали. **Это архитектурная проблема промпта**, не модели. Решения: (A) prompt engineering с явным запретом fabrication, (B) structured output через `generateObject` + Zod schema с URL как `z.enum([...allowedUrls])` — физически не позволит модели генерировать URL вне списка, (C) post-validation + replacement fabricated URLs. Рекомендуется B как long-term fix. Product quality risk — критично. | 1-2 сессии | Владимир + DevPanel Pipeline Trace + empirical test, 2026-04-16 |
