...
### Решение (принято 2026-04-18)

**Порядок серий после закрытия xAI-миграции и AISDK hardening:**

```
1. ТЗ-COMPACTION-1 (MVP для expertise/create)
2. ТЗ-XAI-COL-1 (Collections / Библиотека)
3. ТЗ-XAI-MA-1 (Multi-Agent Экспертиза)
4. ТЗ-XAI-VOICE-1 (Voice Agent)
```

COMPACTION-2 (расширение на Simply Chat) и COMPACTION-3 («Новый чат с итогом») — не независимые серии, вплетаются по мере необходимости после COMPACTION-1.

### Обоснование порядка (владелец, 5 причин)

1. **Архитектурный документ v1.0 уже фиксирует это место.** В секции «Приоритет реализации» прямо написано: «После завершения серии XAI-4…6 → ТЗ-COMPACTION-1 MVP для expertise/create». Документ утверждён 2026-04-17. Серия xAI закрыта 2026-04-16. Место — сейчас.
2. **Compaction — фундамент для COL-1.** Collections = пользователь загружает большие документы в чат. Без compaction контекст переполнится на первом же крупном документе, Grok-чаты молча обрежут начало разговора (секция «Проблема» архитектуры). Делать COL-1 раньше = выкатить фичу, которая ломается в первом сценарии использования. Антипаттерн.
3. **Реальная UX-деградация в 3 chat-режимах.** Expertise, Create, Projects-on-Grok сегодня молча теряют начало разговора при переполнении — у xAI нет аналога Anthropic Compaction API. Это не теоретическая проблема, это документ описывает как критическую.
4. **MA-1 и VOICE-1 независимы от Compaction.** MA-1 использует Responses API с другим форматом messages, compaction для Chat Completions с ним не пересекается. VOICE-1 — отдельный канал. Порядок между ними — продуктовый вопрос, не технический.
5. **COL-1 впереди MA-1 — продуктовая ценность.** COL-1 закрывает ключевой пункт продуктового видения (загрузка документов в RAG). Grok Collections API из коробки, интеграция понятна. MA-1 требует отдельный MCP-сервер + auth layer — больше работы, меньше продуктовой ценности на первом шаге. Сначала то, за что пользователь будет платить.

### Технические подтверждения (архитектор, 7 фактов)

1. ADR 053 создан сегодня — расширение до 5 аспектов (5-й: compaction strategy) вносить пока контракт свежий.
2. Архитектурный документ v1.0 готов — ресёрч-слой пройден, остаётся техническое проектирование + реализация.
3. 0 открытых ТЗ в работе — чистое окно.
4. TZ_SimplyContextUsageWidget уже помечен superseded COMPACTION-1 (прямо в документе v1.0 строки 251-259). Косметический хвост виджета закрывается попутно.
5. Историческая спека [TZ_RAG3_COMPACTION_SPEC.md](../TZ_RAG_SimplyRAG/TZ_RAG3_COMPACTION_SPEC.md) от 2026-04-07 описывала Anthropic Compaction API. После миграции частично устарела. Capability-driven подход v1.1 естественно покрывает оба пути: на Anthropic — провайдерский Compaction, на Grok — наш Summary Buffer.
6. Chat Completions — основа (решение #1 от 2026-04-14). COMPACTION-1 работает в этой парадигме, не требует Responses API.
7. Экономика сжатия пренебрежима: 0.5-2.2₽ за одно сжатие на Grok 4.1 Fast (из документа v1.0).

### MVP scope (предложение)

- Фазы 0-1 из архитектуры (обычная работа + первое сжатие)
- Фазы 2-3 (повторное сжатие, «новый чат с итогом») — отложены на COMPACTION-2 и COMPACTION-3
- Пилот — `expertise` (один контролируемый контекст, смоук-тест на 20+ сообщений с вложением)
- Simply Chat compaction поверх MIND — НЕ в MVP (это COMPACTION-2, MIND-extract 60%/80% доказан)

### Риски, учесть заранее

1. Подсчёт токенов должен быть надёжным — документ прямо пишет это как зависимость.
2. Schema migration для `compaction_summary/index/count` — hard-to-reverse, без rollback-сценариев.
3. Старые сообщения остаются в UI видимыми — значит БД хранит их, из LLM-history исключаются по `compaction_index`. Middleware `applyContextStrategy` смотрит и БД, и индекс.
4. MIND + Compaction в Simply Chat — открытый вопрос #5 документа v1.0. Решить в Фазе 1 MVP (рекомендация архитектора: отложить на COMPACTION-2).

### Что делает архитектор в следующей сессии

1. Проверяет чтение ИТОГ + получает от владельца подтверждение выбранного направления (ТЗ-COMPACTION-1).
2. Готовит короткое задание Claude Code на Фазу 1 (только ANALYSIS + вопросы, код не трогаем):
   - Первый шаг: дополнить [SIMPLY_COMPACTION_ARCHITECTURE.md](SIMPLY_COMPACTION_ARCHITECTURE.md) до v1.1 секцией «Провайдер-агностичность» перед «Техническая реализация» (7 пунктов из описания владельца).
   - Второй шаг: ANALYSIS кодовой базы (chat handlers, usage counting, схема Chat таблицы) + ответы на 5 открытых вопросов документа v1.0 с рекомендациями + список новых вопросов к архитектору.
3. Не пишет код за Claude Code. Не перепрыгивает на реализацию.
4. Ждёт ANALYSIS, проверяет, отвечает на вопросы, утверждает SPEC.

### Правило серии

Правило «один ТЗ в работе» — в силе. Пока ТЗ-COMPACTION-1 не закрыт → MA-1 / COL-1 / VOICE-1 не обсуждаются.
