...
**Версия:** 1.10 (терминология виджета: «чат» → mode-aware «запрос/задание»)
**Дата:** 2026-04-19 (v1.10 — правка терминологии по замечанию владельца в Этапе A6: в expertise/create по продуктовому решению «чат» не употребляется — используются «запрос» / «задание». Мои v1.8 формулировки в таблице виджета контекста «Рекомендуем начать новый чат» / «Новый чат с итогом» были нарушением этой терминологии)
**Статус:** Утверждена (архитектурное решение)
**Автор:** Архитектор (Claude Opus)
**Связанные документы:** MIND_ARCHITECTURE.md, SIMPLY_ATTACHMENT_ARCHITECTURE.md, ADR 052, ADR 053
...
**Может повторяться многократно.** Разрыв Soft (100K) ↔ Hard (170K) = 70K пространства для 2-3 повторных циклов. Каждый цикл «продлевает жизнь» разговора. Когда цикличное сжатие перестаёт спасать — срабатывает Фаза 3.

#### Фаза 3 — Рекомендация нового чата (Hard trigger — 85%)

**Триггер:** usage ≥ **85% от `SIMPLY_CONTEXT_LIMIT`** = **170 000 токенов**. Типичный сценарий — после нескольких циклов Фазы 2 контекст всё равно критически полон (огромные вложения, экстремально длинный разговор). Headroom 30K токенов от Hard до `SIMPLY_CONTEXT_LIMIT` (200K) — запас на генерацию финального summary + output-ответ.
...
| Тип события | Визуал | Текст (общий) | Действие пользователю |
|---|---|---|---|
| `extract` | 🧠 Зелёный/нейтральный | «Записал в память» | Ничего не нужно |
| `compaction` | 📦 Нейтральный | «Разговор сжат» | Ничего не нужно |
| `truncation_warning` | ⚠️ Жёлтый | «Рекомендуем начать новый {mode-термин}» | Кнопка mode-aware (см. ниже) |
| `provider_compaction` | 📦 Минимальный | — (может не показывать) | Ничего не нужно |

**Терминология `truncation_warning` (правка v1.10):** в expertise/create **не используется слово «чат»** — продуктовое решение (см. [components/app-sidebar.tsx:97-103](../../components/app-sidebar.tsx#L97-L103)). Label зависит от `chatMode`:

| chatMode | Warning-текст | Label кнопки |
|---|---|---|
| `expertise` | «Рекомендуем начать новый запрос» | «Новый запрос с итогом» |
| `create` | «Рекомендуем начать новое задание» | «Новое задание с итогом» |
| иначе (fallback) | «Рекомендуем начать заново» | кнопка скрыта |

Канонический механизм кнопки: `generateUUID() + getChatUrl(newId, chatMode) + router.push(newChatUrl) + router.refresh()` — тот же паттерн что в sidebar'е приложения. В MVP это **basic action** без pre-fill summary; полная реализация «Новый {режим} с итогом» (инжект summary в pre-filled input) — отдельный ТЗ-COMPACTION-3.
...
- **Никогда:** красные зоны, мигание, слово «опасность», технические термины, слово «чат» в контекстах expertise/create/projects
