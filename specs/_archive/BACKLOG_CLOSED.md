...
| TZ_SimplyChatLoadPerf | 2026-04-28 | v3.100.4 / `03ae49d` | Fix: удалён лишний `useSWR` в `components/document-preview.tsx` — рендер списка сообщений делал 8 параллельных `GET /api/document` без необходимости. TTI 15-22с → 0.35с (горячий кэш). | [_archive/TZ_SimplyChatLoadPerf/](TZ_SimplyChatLoadPerf/) |
| TZ_ChatModeUndefinedSubmit | 2026-04-28 | v3.100.4 / `92af88e` | Fix: контракт `chatMode?: string` → required в `MultimodalInputProps`, проброс через `<Artifact>` в `chat.tsx` + `task-chat.tsx`. Бонус: `<ArtifactMessages>` переписан на `<Conversation>` со `StickToBottom` — автоскролл и кнопка «↓». | [_backlog/_archive/TZ_ChatModeUndefinedSubmit.md](../_backlog/_archive/TZ_ChatModeUndefinedSubmit.md) |
| TZ_ChatInputBlockedOnDocumentFetchHang | 2026-04-28 | v3.100.4 / `a4e1c23` | Fix: `fetcherWithTimeout(5000)` в `lib/utils.ts`, применён в SWR-загрузке `<Artifact>`. Worst case 12s до error state вместо infinite hang. | [_backlog/_archive/TZ_ChatInputBlockedOnDocumentFetchHang.md](../_backlog/_archive/TZ_ChatInputBlockedOnDocumentFetchHang.md) |
| TZ_PptxRevealUpdateRender | 2026-04-28 | v3.100.4 / `a449fac` | Fix: useEffect в `components/artifact.tsx` на переход `streaming → idle` зовёт `mutateDocuments()`. Корень: `useSWR(/api/document?id=X)` после возврата в idle отдавал старый кэш. Универсально для всех типов артефактов. | [_backlog/_archive/TZ_PptxRevealUpdateRender.md](../_backlog/_archive/TZ_PptxRevealUpdateRender.md) |
