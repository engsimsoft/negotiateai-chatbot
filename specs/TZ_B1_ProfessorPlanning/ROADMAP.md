...
**Задачи:**
- [ ] Рефакторить `planning-state.tsx` (убрать заглушку): принимает props `projectId`, `planJson`, `planReport`
- [ ] **Состояние 1 — Прогресс:** Анимация 6 шагов (аудит данных → ресурсы → декомпозиция → зависимости → риски → финализация). Каждый шаг появляется по таймеру (~12 сек). При получении реального ответа — все шаги мгновенно завершаются.
- [ ] **Состояние 2 — Вопросы (status: needs_input):** Список вопросов с текстовыми полями + кнопка "Отправить ответы". При отправке → повторный POST /plan с userAnswers → снова прогресс.
- [ ] **Состояние 3a — План готов (status: complete):** Карточки задач (order, title, goal, tools, needsReview). Блок рисков. Блок рекомендаций. Раскрываемый planReport (Markdown). Кнопки: "Утвердить" (disabled + tooltip) и "Обсудить с Менеджером" (→ открывает Manager Drawer).
- [ ] **Состояние 3b — План с оговорками (status: partial):** Как 3a + дополнительный блок "⚠️ Профессор рекомендует уточнить" с caveats (затронутые задачи, проблема, последствия, вопрос).
- [ ] Обновить `project-work-area.tsx`: передавать planJson и planReport в PlanningState
- [ ] Подключить PlanningState к POST /plan: useEffect при mount → если planJson отсутствует → автоматический POST /plan → показать прогресс
...
- [ ] Браузер: перейти в проект → "Начать планирование" → видна анимация прогресса → появляется план
- [ ] Браузер: обновить страницу — план виден без повторного запроса (данные из БД)
- [ ] Браузер: если questions — видны поля ввода и кнопка
- [ ] Браузер: "Утвердить" = disabled, "Обсудить" = открывает drawer
- [ ] 🧪 Мануальный тест пользователем: полный flow от кнопки до плана
...
- [ ] 🧪 Мануальный тест пользователем: E2E от создания проекта до просмотра плана в Пульсе
