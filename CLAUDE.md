...
**Проект:** Simply | **Версия:** 3.60.0 | **Статус:** Active development
...
**Meeting Recorder (v3.61.0 — ТЗ-MR):**
...
- `lib/meeting/meeting-pipeline.ts` — Core pipeline (Deepgram Nova-3 transcription → Claude Sonnet summarization → DB save → blob cleanup)
...
- `lib/db/schema.ts` — +meetingRecord таблица (userId FK, title, durationSeconds, speakerCount, summaryLevel, transcript, summary, metadata JSONB)
...
- `components/meeting/meeting-page.tsx` — Страница записи (state machine: input→ready→uploading→processing→result→viewing)
- `app/(chat)/api/meeting/upload/route.ts` — POST endpoint: upload audio → Vercel Blob
- `components/meeting/meeting-progress.tsx` — UI прогресса pipeline (steps, animation, error/retry)
- `components/meeting/meeting-result.tsx` — Результат (MarkdownViewer, metadata pills, кликабельные таймкоды, аудио-плеер, copy/delete)
