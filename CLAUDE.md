...
**Проект:** Simply | **Версия:** 3.58.0 | **Статус:** Active development
...
**Developer Panel (v3.57.0 — ТЗ-DEV1):**
- `lib/ai/debug-events.ts` — Типы + emit functions (DebugStepData, DebugFinishData, DebugGuardianData, DebugPromptData). Guard: isSimplyDevMode
- `lib/ai/providers.ts` — +MODEL_PRICING_RUB + calculateCostRub() (стоимость запроса в рублях)
...
- `components/dev-panel/dev-panel-footer.tsx` — Компактная строка под AI-ответом (модель · токены · ₽стоимость · время), live elapsed timer, error state (красный)
- `components/dev-panel/dev-panel-drawer.tsx` — Sheet справа (w-full sm:w-[440px]) с 6 секциями
- `components/dev-panel/sections/` — model-section, tokens-section, timeline-section, guardian-section, prompt-section, raw-section
