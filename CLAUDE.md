...
**Проект:** Simply | **Версия:** 3.71.0 | **Статус:** Active development
...
**MIND Memory / RAG (v3.70.0 — ТЗ-RAG0, v3.71.0 — ТЗ-RAG1):**
...
- `lib/ai/memory/extract.ts` — extractFactsFromMessages (Claude Sonnet + generateObject), extractAndStoreFacts (extract → embed → dedup → upsert) ← v3.71.0
...
- `lib/db/schema.ts` — +customType vector(1024), +memoryEntry table (HNSW index)
