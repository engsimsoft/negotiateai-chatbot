...
**Проект:** Simply | **Версия:** 3.75.0 | **Статус:** Active development
...
- Anthropic Claude — основной и единственный провайдер (Gemini только для vision-ocr и briefing-фильтра)
...
**AI:**
- Vercel AI SDK v6 (@ai-sdk/anthropic v3, @ai-sdk/google v3 для vision-ocr)
...
**Simply Chat (v3.75.0 — ТЗ-KITT + ТЗ-SaveFact):**
...
- `lib/prompts/chat/simply-chat.md` — System prompt Simply Chat (роль, навигация, память MIND)
- `lib/ai/tools/save-fact.ts` — Tool saveFact: гарантированная запись факта в MIND (embedding + dedup + insert, source="explicit")
...
**Context Window Management (v3.18.0 — ТЗ-C1.5, v3.73.0 — ТЗ-RAG3 Compaction, v3.76.0 — ТЗ-SlidingWindow):**
- **Sliding Window** (v3.76.0) — Simply Chat отправляет в API только последние 20 сообщений (`SIMPLY_SLIDING_WINDOW_SIZE`). Старые сообщения остаются в БД (скролл). MIND + Opus-профиль покрывают долговременную память
...
- `lib/ai/context-limits.ts` — Конфиг (CONTEXT_BUDGET, SIMPLY_SLIDING_WINDOW_SIZE, SNAPSHOT_THRESHOLD, FALLBACK_MESSAGE_PAIRS, trimToUserStart)
...
**MIND Memory / RAG (v3.70.0 — ТЗ-RAG0, v3.71.0 — ТЗ-RAG1, v3.72.0 — ТЗ-RAG2):**
...
- `lib/ai/memory/extract.ts` — extractFactsFromMessages (Claude Sonnet + generateObject), extractAndStoreFacts (extract → embed → dedup → upsert), мини-консолидация триггер ← v3.72.0, двухуровневая дедупликация (embedding 0.55 + Haiku LLM verify) ← v3.73.0
...
- `app/api/cron/memory-profile/route.ts` — Ночной cron: консолидация (Sonnet) → профиль (Opus), 0:00 UTC ← v3.72.0
...
- `lib/briefing/briefing-config.ts` — Константы (лимиты, таймауты, модели Gemini, CRON_INTERVAL_MINUTES, CRON_CONCURRENCY_LIMIT)
...
**Podcast Engine (v3.43.0 — ТЗ-Б1):**
- `lib/podcast/index.ts` — Public API: generatePodcastSegment()
- `lib/podcast/script-generator.ts` — Gemini 2.5 Flash: generateScript() (@ai-sdk/google)
- `lib/podcast/tts-gemini.ts` — Gemini 2.5 Flash TTS: synthesizeSpeech() (@google/genai, multi-speaker: Kore + Puck)
- `lib/podcast/audio-converter.ts` — PCM → MP3 (lamejs, lazy loading + pnpm path resolution) ← v3.55.1 ADR 027
- `lib/podcast/audio-merger.ts` — Склейка MP3 треков + upload в Vercel Blob ← v3.55.1
- `lib/podcast/types.ts` — Все типы (VoiceConfig, PodcastSegment, AudioResult, PodcastProgressEvent)
- `lib/podcast/lamejs.d.ts` — TypeScript declarations для lamejs
- `lib/podcast/podcast-pipeline.ts` — Core podcast pipeline (browser + background) ← v3.54.0
- `app/(chat)/api/briefing/podcast/generate/route.ts` — Streaming POST endpoint (тонкая обёртка вокруг podcast-pipeline)
- `lib/prompts/briefing/briefing-scriptwriter.md` — System prompt скриптрайтера
- `lib/prompts/briefing/briefing-scriptwriter-user-template.md` — Шаблон user message
