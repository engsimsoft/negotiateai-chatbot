...
- Anthropic Claude — основной провайдер, MiniMax — Simply Chat + Briefing + Podcast
...
**AI:**
- Vercel AI SDK v6 (@ai-sdk/anthropic v3, vercel-minimax-ai-provider)
...
**Podcast Engine (v3.43.0 — ТЗ-Б1, v3.81.0 — ТЗ-Briefing-2 MiniMax):**
- `lib/podcast/index.ts` — Public API: generatePodcastSegment() (M2.7 script → Speech 2.8 HD TTS)
- `lib/podcast/script-generator.ts` — MiniMax M2.7: generateScript() — JSON+plain text universal parser ← v3.81.0
- `lib/podcast/tts-minimax.ts` — MiniMax Speech 2.8 HD: generateSpeech(), generateReplicasSpeech() — per-replica TTS, pLimit(4), паузы `<#0.3#>` ← v3.81.0
- `lib/podcast/audio-merger.ts` — Склейка MP3 треков + upload в Vercel Blob ← v3.55.1
- `lib/podcast/types.ts` — Все типы (VoiceConfig, ScriptLine, PodcastSegment, AudioResult, PodcastProgressEvent)
...
- `lib/prompts/briefing/briefing-scriptwriter.md` — System prompt скриптрайтера (v2: JSON формат, интонационные теги, антипаттерны) ← v3.81.0
