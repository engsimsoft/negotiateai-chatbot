...
### 1.5 Терминология режимов — НЕ «чат» в expertise/create/projects

⛔ **ОБЯЗАТЕЛЬНО.** Слово «чат» — это словарь генерик-чатботов (ChatGPT, DeepSeek, Claude.ai, Perplexity и пр.). **Simply — другой продукт**, и в его UX для большинства режимов «чат» **не употребляется**. Каждый режим — отдельная продуктовая сущность со своей терминологией:

| chatMode | Пользовательский термин сессии | Кнопка «создать новую» | Страница списка |
|---|---|---|---|
| `simply` | **чат** (Simply Chat — один вечный на userId) | не создаётся пользователем | «Simply» |
| `expertise` | **запрос** | «Новый запрос» | «Запросы» / «Все запросы» |
| `create` | **задание** | «Новое задание» | «Задания» / «Все задания» |
| `task` (projects) | **задача** | (создаётся в проекте) | (в контексте проекта) |

**Запрещено** в user-facing UI строках (`expertise` / `create` / `projects`):
- «новый чат», «все чаты», «N чатов», «диалог», «разговор»
- «Начать новый чат», «Открыть чат», «О чём чат»

**Разрешено** и обязательно:
- `expertise`: «Новый запрос», «Открыть запрос», «О чём запрос», «N запросов»
- `create`: «Новое задание», «Открыть задание», «О чём задание», «N заданий»
- `projects`: «задача» (в контексте проекта)

**Why:** Simply — не чатбот. Каждый режим — продуктовая сущность: Экспертиза = экспертный **запрос** и ответ эксперта; Создать = рабочее **задание** на генерацию материала; Проект = структурированная **задача** в составе проекта. Метафора «чата» ломает UX-модель этих сущностей. Решение зафиксировано в `_archive/TZ_DV2U_DashboardV2Updated` (дни согласования) и `_archive/TZ_LegacyChatCleanup`.

**SSOT label'ов:**
- Кнопка «создать» → [components/app-sidebar.tsx:97-103](../components/app-sidebar.tsx#L97-L103) (`getNewChatLabel`).
- Страница списка → [components/app-sidebar.tsx:91-94](../components/app-sidebar.tsx#L91-L94) (`getContextTitle`), [:113-118](../components/app-sidebar.tsx#L113-L118) (`getAllChatsLabel`).
- URL-механика для «создать новую» → `generateUUID() + getChatUrl(newId, chatMode) + router.push + router.refresh` (паттерн `app-sidebar.tsx:79-86`).

**Технический уровень НЕ затрагивается:** `chatMode`, `getChatUrl`, `ChatMessage`, таблица БД `Chat` — остаются как есть (это internal namespace, не видно пользователю). Правило касается только **видимых строк** в UI.

**Default fallback-тексты «Новый чат» в коде** остаются для unknown-контекстов (см. ТЗ_LegacyChatCleanup) — не регрессия.
