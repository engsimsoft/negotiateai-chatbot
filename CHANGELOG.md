...
- **Что изменилось у пользователя:** превью файла после reload остаётся карточкой вложения с именем файла — не разворачивается в портянку.

- **Архитектура:** [components/message.tsx](components/message.tsx) — детектор text-part-маркера `^📄 \*\*Файл: ([^*\n]+)\*\*\n\`\`\`` для user-сообщений. Если совпадает — рендер через `<PreviewAttachment>` вместо markdown `<Response>`. Узкий UI-фикс, не трогает persistence/payload pipeline.

- **Запланированное удаление:** этот детектор станет мёртвым кодом после Шага 4 миграции (PDF на xAI Files API → файлы идут как `file_id`, в БД будут file parts, карточка отрисуется через стандартный `attachmentsFromMessage` блок). Чек-лист удаления переходного кода добавлен в [SIMPLY_MIGRATION_CONCEPT.md Блок 3](specs/Simply_Migration/SIMPLY_MIGRATION_CONCEPT.md). TODO-маркер в коде ссылается на тот же блок.

- **Корневая причина (для контекста):** [app/(chat)/api/chat/route.ts:652+731-744](app/(chat)/api/chat/route.ts#L652) — `convertTextFilePartsInMessage` конвертит file→text **до** save в БД (фикс xAI SDK от 2026-04-15: SDK ломался на text/plain file parts). Архитектурно правильное решение — хранить оригинальный file part и конвертировать только на лету для payload модели — закладывается в Шаг 4 миграции.

- **Версия проекта:** 3.100.4 → 3.100.5 (patch — точечный UI-фикс, без поведенческих изменений)
...
- **[CLAUDE.md](CLAUDE.md): 653 → 211 строк.** Удалена 18-КБ суперстрока в блоке «Завершены» с перечислением 90+ ТЗ от v3.0.0 до v3.90.0 (дубликат истории из этого CHANGELOG). Секция «Структура кода» сжата с 395 строк пофайловой летописи с версионными тегами до ~55 строк высокоуровневой карты со ссылками на `docs/architecture.md` и тематические `docs/ai-*.md`. Секция «Workflow» сжата с 67 до 17 строк — оставлены только критические правила, детали в `specs/WORKFLOW.md`. Устаревшие упоминания моделей в «О проекте» (MiniMax как основной — неверно после xAI миграции) заменены на ссылку на `docs/ai-chats-map.md`. В шапку добавлен **инвариант-блок**: лимит 220 строк, запрет на историю ТЗ / версионные теги / пофайловую карту / конкретные модели.
- **Root cause** деградации — не халатность исполнителя, а процессный конвейер: [specs/WORKFLOW.md](specs/WORKFLOW.md) Правило 6 (L124) прямо требовал верифицировать `CLAUDE.md → Структура кода` против `ls/Glob`; два чеклиста финализации (L611, L685) требовали «Обновлён CLAUDE.md» без указания ЧТО писать / что НЕ писать; [specs/ROADMAP_GUIDE.md](specs/ROADMAP_GUIDE.md) (L226) в шаблоне финализации содержал `- [ ] Обновить CLAUDE.md (если менялась структура)`; ROADMAP конкретных ТЗ внешнего архитектора Simply_xAI (`TZ_xai_2/ROADMAP.md:105`, `TZ_xai_3/ROADMAP.md:168`) явно прописывали «добавить запись в «Завершены»». За ~90 ТЗ этот конвейер разогнал файл в 3× от нормы.
- **Процесс исправлен в трёх местах:**
  - [specs/WORKFLOW.md](specs/WORKFLOW.md): L124 таблица Правила 6 — `CLAUDE.md → Структура кода` заменён на `docs/architecture.md → Модули и слои` (пофайловая карта живёт там). L611, L685 — «Обновлён CLAUDE.md» заменено на запретительную формулировку с обязательным `wc -l CLAUDE.md ≤ 220`. L631, L691 — аналогично.
  - [specs/ROADMAP_GUIDE.md](specs/ROADMAP_GUIDE.md): L226 и L334 (пример) — тот же запрет + обязательная проверка `wc -l`.
  - [DOCUMENTATION_GUIDE.md](DOCUMENTATION_GUIDE.md): расширена секция «CLAUDE.md (Навигация для AI)» — добавлен жёсткий лимит 220 строк, явный список «что НЕ ДОЛЖНО быть» (история ТЗ, версионные теги, пофайловая карта, конкретные модели), правило финализации, и урок-памятка 2026-04-15 про root cause. L360 чеклист «Добавил новый компонент/хук → Обнови CLAUDE.md» заменён на запрет (идёт в `docs/architecture.md`).
- **Ретроспективно не трогали** ROADMAP конкретных уже запущенных ТЗ (append-only принцип), CHANGELOG, SIMPLY_STATUS.md, docs/architecture.md — это отдельные задачи при необходимости.
- **Защита от регрессии:** правило теперь прописано в 4 местах — сам CLAUDE.md (инвариант в шапке, виден первым при открытии), WORKFLOW.md (процессный чеклист финализации), ROADMAP_GUIDE.md (шаблон), DOCUMENTATION_GUIDE.md (главный документ-регулятор). Цифра 220 синхронизирована между всеми четырьмя. Первая полезная нагрузка — следующий ТЗ серии Simply_xAI физически не сможет выполнить «добавить запись в Завершены», потому что такого пункта больше нет в шаблонах.

**Версию проекта не поднимаем** — это docs/chore, не продуктовое изменение. Бэкап `CLAUDE.md.bak` (старые 653 строки) сохранён на время review diff, владелец удалит после одобрения.
